...
```bash
sam_intro$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam_intro$ sam local start-api
sam_intro$ curl http://localhost:3000/classify_digit
```
...
```bash
sam_intro$ sam logs -n InferenceFunction --stack-name sam_intro --tail
```
...
```bash
aws cloudformation delete-stack --stack-name sam_intro
```
